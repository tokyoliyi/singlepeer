...
func (token *Token) initialSupply(_name string, _symbol string, _supply float64, _account *Account, lock bool) []byte {
	...
	if _account.BalanceOf[_symbol] > 0 {
		...
		if _account.BalanceOf == nil {
			_account.BalanceOf = map[string]float64{}
		}
...
//添加转账记录
func (token *Token) installRecord(_from string, _to string, _currency string, _value float64, _memo string, tsSec int64, txId string) []byte {

	var curr Currency

	record := TransactionRecord{_from, _to, _value, _memo, tsSec, txId}
...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "initLedger" {
		...
		// peer chaincode invoke -C myc -n token -c '{"function":"initCurrency","Args":["Netkiller Token","VKT","1000000","greavankia","false"]}'
		return s.initCurrency(stub, args)
	} else if function == "setLock" {
		//锁仓某个代币 (1) 代币简称 (2) 是否锁仓 (3) 操作人
		//peer chaincode invoke -C myc -n token -c '{"function":"setLock","Args":["VKT","true","greavankia"]}'
		return s.setLock(stub, args)
	} else if function == "transferToken" {
		//转账 (1) 发送人(2) 接收人(3) 代币名(4)发送代币量
		//peer chaincode invoke -C myc -n token -c '{"function":"transferToken","Args":["greavankia","123","ada","12.584","memo"]}'
		return s.transferToken(stub, args)
	} else if function == "frozenAccount" {
		//冻结账户 (1) 要冻结的账户 (2) 是否冻结 (3) 操作人
		//peer chaincode invoke -C myc -n token -c '{"function":"frozenAccount","Args":["netkiller","true","greavankia"]}'
		return s.frozenAccount(stub, args)
	} else if function == "mintToken" {
		//代币增发 (1)代币名称(2)增发数量(3)操作人，也是代币增发接收人
		//peer chaincode invoke -C myc -n token -c '{"function":"mintToken","Args":["VKT","5000","greavankia"]}'
		return s.mintToken(stub, args)
	} else if function == "burnToken" {
		//代币销毁 (1)代币名称(2)回收数量(3)回收的账户（回收谁的代币）(4)操作人
		//peer chaincode invoke -C myc -n token -c '{"function":"burnToken","Args":["VKT","5000","123","greavankia"]}'
		return s.burnToken(stub, args)
	} else if function == "balance" {
		//查询指定账户指定代币 (1)查询账户 （2） 代币名称
		//peer chaincode invoke -C myc -n token -c '{"function":"balance","Args":["greavankia","VKT"]}'
		return s.balance(stub, args)
	} else if function == "tokenHistory" {
		//查询指定代币交易记录 (1)代币名称
		//peer chaincode invoke -C myc -n token -c '{"function":"tokenHistory","Args":["VKT"]}'
		return s.tokenHistory(stub, args)
	} else if function == "userTokenHistory" {
		//查询指定用户指定代币交易记录 (1)代币名称(2)用户名
		//peer chaincode invoke -C myc -n token -c '{"function":"userTokenHistory","Args":["VKT","greavankia"]}'
		...
		//查询某个用户所有资金 (1)账户名
		//peer chaincode invoke -C myc -n token -c '{"function":"balanceAll","Args":["greavankia"]}'
		return s.balanceAll(stub, args)
	} else if function == "showAccount" {
		//查看某个账户(1)账户名
		//peer chaincode invoke -C myc -n token -c '{"function":"showAccount","Args":["greavankia"]}'
		return s.showAccount(stub, args)
...
func (s *SmartContract) transferToken(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if msg.Status == true {
		_timestamp, _ := stub.GetTxTimestamp()
		result = token.installRecord(fromAccount.Name, toAccount.Name, _currency, _amount, _memo, _timestamp.GetSeconds(), stub.GetTxID())
		fmt.Printf("Result %s \n", string(result))
		tokenAsBytes, _ = json.Marshal(token)
		err = stub.PutState(TokenKey, tokenAsBytes)
		if err != nil {
...
func (s *SmartContract) balanceAll(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	resultAsBytes, _ := json.Marshal(result)
	fmt.Printf("%s account.Account is %s \n", _account, string(account.Name))
	fmt.Printf("%s account.Account is %f \n", _account, float64(account.BalanceOf["VKT"]))
	fmt.Printf("%s balance is %s \n", _account, string(resultAsBytes))
